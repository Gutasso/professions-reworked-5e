...
export const Engenheiro = {
    ...
    onCreateProject(projectData, container, { actor }) {
        ...
        if (subTipo === "Mecanismo Improvisado" || subTipo === "Conserto") {
    ...
    prepareProject(projeto, comp, { actor }) {
        ...
        if ((projeto.subTipo === "Mecanismo Improvisado" || projeto.subTipo === "Conserto") && projeto.complexidade) {
            return {
                infoExtra: ` (${projeto.complexidade} - ${projeto.dificuldadeEspecifica || (comp ? comp.dificuldade : "N/A")})`,
                totalNecessario: projeto.acertosTotaisPreparo || (comp ? Math.ceil(comp.acertosNecessarios / 2) : 5)
            };
        }
        if ((projeto.subTipo === "Mecanismo" || projeto.subTipo === "Planos de Estruturas") && projeto.complexidade) {
            return {
                infoExtra: ` (${projeto.complexidade} - ${projeto.dificuldadeEspecifica || (comp ? comp.dificuldade : "N/A")})`
            };
        }
